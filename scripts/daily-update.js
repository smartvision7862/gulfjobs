/**
 * daily-update.js
 * Run by GitHub Actions every day at 6 AM UTC (or manually).
 * 
 * What it does:
 *  1. Scrapes latest live jobs from Swan Global (https://swanglobal.careers-page.com) into jobs-pool.json
 *  2. Reads jobs-data.json
 *  3. Removes jobs older than MAX_AGE_DAYS (8 days)
 *  4. Picks NEW_JOBS_PER_DAY jobs from jobs-pool.json (round-robin)
 *  5. Assigns today's dateAdded and "Today" postedDate to new jobs
 *  6. Saves updated jobs-data.json
 *  7. Updates pool-index.json to track which pool jobs have been used
 *  8. Generates feed.xml for Indeed and LinkedIn XML feeds
 */

const fs = require('fs');
const path = require('path');
const { scrapeSwanGlobal } = require('./fetch-swanglobal');
...
async function runDailyUpdate() {
    ...
    // 0. Scrape live Swan Global jobs first
    try {
        await scrapeSwanGlobal(3);
    } catch (e) {
        console.warn('⚠️ Warning: Swan Global live scraping encountered an issue, continuing with pool rotation.', e.message);
    }
    ...
    for (let attempts = 0; attempts < pool.length && added < NEW_JOBS_PER_DAY; attempts++) {
        ...
        // Give it a fresh ID based on timestamp if needed so it can be re-added in future rotations
        const newJob = {
            ...poolJob,
            id: poolJob.id.startsWith('swan-') ? poolJob.id : `${poolJob.id}-${todayStr}`,
            dateAdded: todayStr,
            postedDate: 'Today',
...
function generateXmlFeed(jobsList, filePath) {
    ...
    jobsList.forEach(job => {
        ...
        xml += `    <url><![CDATA[${job.applyUrl || `https://smartvision7862.github.io/gulfjobs/#job-${job.id}`}]]></url>\n`;
        ...
        if (job.location) {
            ...
            if (parts.length > 1) {
                ...
                if (countryPart === "uae" || countryPart === "united arab emirates") country = "AE";
