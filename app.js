...
async function initJobDatabase() {
    ...
    if (savedDb) {
        try {
            ...
            userPostedJobs = parsed.filter(j => j.userPosted || (j.id && j.id.startsWith("job-") && !isNaN(j.id.replace("job-", "")) && j.id.replace("job-", "").length > 6));
        } catch (e) {}
    }

    // Combine user posts with fresh sourceJobs from server
    jobDatabase = [...userPostedJobs, ...sourceJobs];

    // Filter out jobs older than 8 days
        ...
        <div class="job-card ${job.featured ? 'featured' : ''}" data-id="${job.id}">
            ...
            <div class="job-info">
                <div class="job-tags">
                    ...
                    ${job.applyUrl ? `<span class="tag" style="background:rgba(59,130,246,0.12);color:#3b82f6;font-size:10px;">⚡ Direct Apply</span>` : (job.applyEmail ? `<span class="tag" style="background:rgba(234,179,8,0.12);color:#eab308;font-size:10px;">📧 Email Apply</span>` : '')}
                </div>
            ...
            <div class="job-actions-area">
                ...
                <div class="job-card-actions">
                    ...
                    ${job.applyUrl
                        ? `<a class="btn-primary" href="${job.applyUrl}" target="_blank" rel="noopener noreferrer" style="text-decoration:none;display:inline-flex;align-items:center;gap:4px;">Apply Direct ↗</a>`
                        : (job.applyEmail
...
window.openJobDetail = function(jobId) {
    ...
    elements.modalTagsContainer.innerHTML = `
        ...
        ${activeJob.applyUrl ? `<span class="tag" style="background:rgba(59,130,246,0.15);color:#3b82f6;">⚡ Direct Employer Opening</span>` : ''}
    `;
    
    // Render Modal Tabs content
    renderModalTabContent();
    
    // Show/hide email & direct apply buttons
    const emailBtn = elements.btnApplyEmail;
    const applyNowBtn = document.getElementById("btn-apply-now");
    
    if (activeJob.applyUrl) {
        if (emailBtn) {
            emailBtn.href = activeJob.applyUrl;
            emailBtn.target = "_blank";
            emailBtn.rel = "noopener noreferrer";
            emailBtn.innerHTML = `<svg style="width:14px;height:14px;flex-shrink:0;" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg> Employer Page ↗`;
            emailBtn.style.display = "flex";
        }
        if (applyNowBtn) {
            applyNowBtn.innerText = "Apply Directly ↗";
            applyNowBtn.onclick = function() { window.open(activeJob.applyUrl, "_blank"); };
        }
    } else {
        if (applyNowBtn) {
            applyNowBtn.innerText = "Apply Now";
            applyNowBtn.onclick = function() { openQuickApply(activeJob.id); };
        }
        if (emailBtn) {
            if (activeJob.applyEmail) {
                ...
                emailBtn.innerHTML = `<svg style="width:14px;height:14px;flex-shrink:0;" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg> Send CV`;
                emailBtn.style.display = "flex";
            } else {
...
function renderModalTabContent() {
    ...
    const bodyHTML = `
        <div id="tab-description" class="tab-content active">
            <div style="background:rgba(37,211,102,0.1);border:1px solid rgba(37,211,102,0.3);border-radius:10px;padding:12px 16px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;">
                <div style="display:flex;align-items:center;gap:10px;">
                    <span style="font-size:22px;">📲</span>
                    <div>
                        <strong style="color:#25d366;font-size:14px;display:block;">Get Daily Job Updates on WhatsApp</strong>
                        <span style="font-size:12px;color:var(--text-secondary);">Receive instant GCC openings matching your profile directly on your phone.</span>
                    </div>
                </div>
                <a href="https://whatsapp.com/channel/0029VaCbxMzGZNCvLvghpd20" target="_blank" rel="noopener noreferrer" style="background:#25d366;color:#fff;font-size:12px;font-weight:600;padding:8px 14px;border-radius:6px;text-decoration:none;white-space:nowrap;box-shadow:0 3px 8px rgba(37,211,102,0.3);">Join Channel ↗</a>
            </div>
            <h3>Role Overview</h3>
